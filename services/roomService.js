...
module.exports = {
      ...
      rClient.set(
        ...
        (err, reply) => {
          ...
          if (reply === "OK") {
            ...
            roomName != undefined
              ? resolve(roomName)
              : reject(new Error("No more rooms available"));
          }
          if (reply === null) {
  ...
  joinRoom: async (socket, roomToJoin) => {
    return new Promise(function (resolve, reject) {
      ...
      io.of("/").adapter.remoteJoin(socket.id, roomToJoin, (err) => {
        ...
        err
          ? reject(new Error(ErrorMessage.unknownId))
          : resolve(SuccessMessage.joinRoom);
      });
    });
  },
  /**
   ...
   * Alternative approach to save username within the game object as JSON in Redis
   * @async
  ...
  getAllClientIdsInRoom: async (roomName) => {
    return new Promise((resolve, reject) => {
      io.of("/").adapter.clients([roomName], (err, clients) => {
        ...
        // console.log(err);
        err || clients === null
          ? reject(new Error(ErrorMessage.getAllClientsInRoom))
          : resolve(clients);
      });
    });
  },
  ...
  mapUserIdWithUsername: async (clientIds, usernames) =>
    new Promise((resolve, reject) =>
      resolve(clientIds.map((i, ind) => ({ id: i, name: usernames[ind] })))
    ),
  /**
